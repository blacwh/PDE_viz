...
for i in size:
    tf.reset_default_graph()
    s1 = np.random.choice(20000, size=i, replace=False)
    ...
    r2.append(err_mean)
    print(i, 'loss: ', loss)
    print(i, 'error: ', err_mean)

with open('result/loss_r_{}.txt'.format(i), 'w') as f1:
    f1.write(str(r1))

with open('result/error_r_{}.txt'.format(i), 'w') as f2:
    f2.write(str(r2))
