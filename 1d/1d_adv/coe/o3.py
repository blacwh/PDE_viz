...
def run_o3(layer):
    class PhysicsinformedNN:
        def __init__(self, x, t, u, layers, activation):
            ...
            # Loss
            self.lam = 10**-6
            self.reg_l1 = self.lam * tf.reduce_mean(tf.math.abs(self.lambda_1)+tf.math.abs(self.lambda_2)+tf.math.abs(self.lambda_3))
        ...
        def net_pde(self, x, t):        
            ...
            f = u_t + lambda_1 * u_x - lambda_2 * u_xx - lambda_3 * u_xxx
        ...
        def train(self, nIter):
            ...
            return loss, self.l1, self.l2, self.l3
    ...
    def fit(model, epochs):
        history, l1, l2, l3 = model.train(epochs)
        loss = history[-1]
        print('history: ', history)
        print('loss: ', loss)
        return loss, float(l1), float(l2), float(l3)


    df = pd.read_csv('adv_1d.csv')
    # layer = [100, 100, 100, 50, 50]
    model = model_execution(layer)
    print('~~~~~start the model: {}(order 3)~~~~~~~~'.format(layer))
    loss, l1, l2, l3 = fit(model, epochs=2000)
    err = abs(np.array([(1 -  l1), (1 - l2), l3]))
    err_mean = err.mean()
