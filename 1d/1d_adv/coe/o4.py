...
def run_o4(layer):
    class PhysicsinformedNN:
        ...
        def net_pde(self, x, t):        
            ...
            f = u_t + lambda_1 * u_x - lambda_2 * u_xx - lambda_3 * u_xxx - lambda_4 * u_xxxx

            return u, f
            # return u, u_t, u_x, u_y, u_z, u_xx, u_yy, u_zz, f

        def callback(self, loss, lambda_1, lambda_2, lambda_3, lambda_4):
        ...
        def train(self, nIter):
            ...
            return loss, self.l1, self.l2, self.l3, self.l4
    ...
    def fit(model, epochs):
        history, l1, l2, l3, l4 = model.train(epochs)
        loss = history[-1]
        print('history: ', history)
        print('loss: ', loss)
        return loss, float(l1), float(l2), float(l3), float(l4)


    df = pd.read_csv('adv_1d.csv')
    # layer = [100, 100, 100, 50, 50]
    model = model_execution(layer)
    print('~~~~~start the model: {}(order 4)~~~~~~~~'.format(layer))
    loss, l1, l2, l3, l4 = fit(model, epochs=2000)
    err = abs(np.array([(1 -  l1), (1 - l2), l3, l4]))
    err_mean = err.mean()
