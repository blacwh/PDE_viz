...
def run_o5(layer):
    class PhysicsinformedNN:
        def __init__(self, x, t, u, layers, activation):
            ...
            # Loss
            self.lam = 10**-6
            self.reg_l1 = self.lam * tf.reduce_mean(tf.math.abs(self.lambda_1)+tf.math.abs(self.lambda_2)+tf.math.abs(self.lambda_3)+tf.math.abs(self.lambda_4)+\
                                                    tf.math.abs(self.lambda_5))
            self.loss_1 = tf.reduce_mean(tf.square(self.u_tf - self.u_pred))
            self.loss_2 = tf.reduce_mean(tf.square(self.f_pred))
            self.loss = self.loss_1 + self.loss_2 + self.reg_l1
            # Optimizers     
        ...
        def net_pde(self, x, t):        
            ...
            f = u_t + lambda_1 * u_x - lambda_2 * u_xx - lambda_3 * u_xxx - lambda_4 * u_xxxx - lambda_5 * u_xxxxx

            return u, f
            # return u, u_t, u_x, u_y, u_z, u_xx, u_yy, u_zz, f

        def callback(self, loss, lambda_1, lambda_2, lambda_3, lambda_4, lambda_5):
        ...
        def train(self, nIter):
            ...
            self.optimizer.minimize(self.sess,
                        feed_dict = tf_dict,
                        fetches = [self.loss, self.lambda_1, self.lambda_2, self.lambda_3, self.lambda_4, self.lambda_5],
            ...
            return loss, self.l1, self.l2, self.l3, self.l4, self.l5
    ...
    def fit(model, epochs):
        history, l1, l2, l3, l4, l5 = model.train(epochs)
        loss = history[-1]
        print('history: ', history)
        print('loss: ', loss)
        return loss, float(l1), float(l2), float(l3), float(l4), float(l5)


    df = pd.read_csv('adv_1d.csv')
    # layer = [100, 100, 100, 50, 50]
    model = model_execution(layer)
    print('~~~~~start the model: {}(order 5)~~~~~~~~'.format(layer))
    loss, l1, l2, l3, l4, l5 = fit(model, epochs=2000)
    err = abs(np.array([(1 -  l1), (1 - l2), l3, l4, l5]))
    err_mean = err.mean()
    print('~~~loss: {} ~~~~'.format(loss))
    print('~~~error: {} ~~~~'.format(err_mean))

    return loss, err_mean